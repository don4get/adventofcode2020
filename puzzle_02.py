...
--- Day 2: Password Philosophy --- Your flight departs in a few days from the coastal
airport; the easiest way down to the coast from here is via toboggan.
...
1-3 a: abcde 1-3 b: cdefg 2-9 c: ccccccccc Each line gives the password policy and
then the password. The password policy indicates the lowest and highest number of
...
--- Part Two --- While it appears you validated the passwords correctly, they don't
seem to be what the Official Toboggan Corporate Authentication System is expecting.
...
def main(policy="old"):
    data = pd.read_csv("puzzle_02_input.csv", names=["range", "letter", "password"], sep=" ")
    values = data.values[0]
    data["letter"] = data["letter"].apply(
        lambda x: x.replace(":", ""))
    # print(data.head())
    number_of_valid_passwords = 0
    for index, v in data.iterrows():
        min, max = [int(val) for val in v["range"].split("-")]
        letter = v["letter"]
        password = v["password"]
